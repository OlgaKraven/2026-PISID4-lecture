...
let notes=[];try{notes=Object.values(JSON.parse(await fs.readFile('private/teacher-pack.json','utf8')).notes).map(n=>n.script);}catch{/* CI checks public structure; local run also checks private notes. */}
for(const p of list){if(/teacher|private|\.map$|source-course|units\.mjs/i.test(p))throw Error('Приватный путь в сборке: '+p);if(/\.(js|html|json|txt)$/.test(p)){const text=await fs.readFile(p,'utf8');for(const note of notes)if(text.includes(note)||text.includes(JSON.stringify(note).slice(1,-1)))throw Error('Сценарий в сборке: '+p);}}
